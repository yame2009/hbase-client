...
import com.tool.hbase.annotation.HBaseColumn;
import com.tool.hbase.annotation.RowKey;
import com.tool.hbase.bean.ColumnInfo;
...
public class HBaseAssistUtil {

	public static <T> List<Put> putObjectList(List<T> objectList) throws Exception {
		...
		for (T obj : objectList) {
			...
			HBaseColumn hBaseColumnForAll = obj.getClass().getAnnotation(HBaseColumn.class);
			...
			for (Field f : fields) {
				...
				boolean isHBaseColumnAnn = true;
				if (hBaseColumn == null) {
					if (hBaseColumnForAll != null) {
						isHBaseColumnAnn = !hBaseColumnForAll.exist();
					}
				} else {
					isHBaseColumnAnn = !hBaseColumn.exist();
				}
					...
					if (value != null) {
						...
						if (StringUtils.isNotBlank(vStr)) {
							...
							if (hBaseColumn != null) {
								...
								if (StringUtils.isNotBlank(hBaseColumn.column())) {
									column = hBaseColumn.column();
								}
							} else if (hBaseColumnForAll != null) {
								columnFamily = hBaseColumnForAll.family();
							}
		...
		if (filters != null) {
			...
			for (ColumnInfo columnInfo : filters) {
				...
				if (null != columnInfo.getValue()) {
					SingleColumnValueFilter filter = new SingleColumnValueFilter(columnFamily.getBytes(), columnInfo.getColumn().getBytes(), compareOp,
