 ...
 * 主要核心代码，封装了总共八大类的方法：
 *
 * 1.获取单个PO的get方法，column用于限制返回的column，filter用于批量过滤 2.获取多个PO的getList方法
 * 3.获取单个signleColumn、多个Column的getColumns方法，支持分页（HBase的宽表分页，基于偏移量设计）
 * 4.支持批量PUT的put方法 5.支持批量DELETE的delete方法 6.支持原子操作的addCounter计数器方法
